...
async function changePassword(request, response, next) {
  try {
    ...
    const isMatched = await passwordMatched(oldPassword, user.password);
    if (!isMatched) {
      throw errorResponder(errorTypes.UNATHORIZED, 'Wrong old password');
    }
    ...
    const result = await usersService.changePassword(id, hashedPassword);
